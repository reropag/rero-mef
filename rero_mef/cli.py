...
@click.option('-a', '--agency', 'agency')
@with_appcontext
def create_or_update(source, verbose, agency):
    """Create or update authority person records."""
    click.secho('Create or update authority person records:', fg='green')
    ...
    for record in data:
            ...
            if status != 'discard':
                click.echo(
                    str(agency) + ' record uuid: ' +
                    str(returned_record.id) + ' | ' + status)
            else:
                id = record['identifier_for_person']
                click.echo(
                    str(agency) + ' record identifier_for_person : ' +
                    str(id) + ' | ' + status)
